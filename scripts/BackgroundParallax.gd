...
func get_object_from_pool(layer_index: int) -> Node2D:
	...
	if object_pools[clamped_layer_index].size() > 0:
		var object = object_pools[clamped_layer_index].pop_back()
		object.visible = true
		
		# Regenerate the object's shape for variety (prevent seeing same shapes repeatedly)
		regenerate_object_shape(object, clamped_layer_index)
		
			# Color is set when object is created, no update needed
		
		return object
	else:
		# If pool is empty, create new object with current colors
		if layer_index == 0:
			return create_foreground_object()
		else:
			return create_random_object(layer_index)
...
func return_object_to_pool(object: Node2D, layer_index: int):
	...
	# Clamp layer_index to valid range to prevent array index errors
	var clamped_layer_index = clamp(layer_index, 0, object_pools.size() - 1)
	object_pools[clamped_layer_index].append(object)
...
func create_foreground_object() -> Node2D:
	...
	polygon_points = generate_complex_shape(shape_type, size)  # regular foreground
...
# Foreground collision handling functions removed - no collisions for decorative debris



func initialize_object_pools():
	# Initialize object pools for each layer with optimized batch creation
	for i in range(layer_speeds.size() + 1):  # +1 for foreground layer
		var pool = []
		var batch_size = 5
		for batch in range(0, pool_size, batch_size):
			var current_batch_size = min(batch_size, pool_size - batch)
			for j in range(current_batch_size):
				var object
				if i == 0:  # Foreground layer
					object = create_foreground_object()
				else:  # Background layers
					object = create_random_object(i - 1)  # i-1 is correct for background layers (0=foreground, 1+=background)
				object.visible = false
				pool.append(object)
...
func reset():
	...
	for i in range(object_pools.size()):
		for object in object_pools[i]:
			if is_instance_valid(object):
				object.queue_free()
		object_pools[i].clear()
...
func regenerate_object_shape(object: Node2D, layer_index: int):
	...
	var shape_type = randi() % 30  # 30 different shapes total
	var polygon_points = generate_complex_shape(shape_type, size)
